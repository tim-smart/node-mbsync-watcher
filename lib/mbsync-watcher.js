 ...
 * @param   Object  options   { channel, maildir, unimportant, important, scripts }
 */
function Mbsyncwatcher (options) {
  ...
  this.on('process:exit', function (mailbox) {
    // run scripts
    watcher.runScripts();
 ...
 */
mwp.runScripts = function runScripts() {
  var self = this

  if (self.scriptState.running) {
    self.scriptState.again = true;
    return;
  }
  self.scriptState.running = true;

  async.each(self.scripts, childprocess.exec, function(err) {
    if (err) {
      self.emit('scripts:error', err);
      return;
    }
    self.emit('scripts:exit');

    self.scriptState.running = false;
    if (self.scriptState.again) {
      self.scriptState.again = false;
      self.runScripts();
    }
  });
...
mwp.loadboxes = function loadboxes (dirpaths) {
  ...
  for (var i = 0, il = dirpaths.length; i < il; i++) {
    ...
    var ignore = watcher.ignore;
    var ignoreLength = ignore.length;
    var toMatch = null;
    var matched = false;
    for (var j = 0; j < ignoreLength; j += 1) {
      toMatch = ignore[j];
      if (0 === box.indexOf(toMatch)) {
        matched = true;
        break;
      }
      break;
    }
    if (matched || !box || boxhash[box]) continue
  ...
  this.mailboxes.forEach(function (mailbox) {
    var toIgnore = watcher.unimportant;
    var toIgnoreLength = toIgnore.length;
    var toMatch = null;
    for (var i = 0; i < toIgnoreLength; i += 1) {
      toMatch = toIgnore[i];
      if (0 === mailbox.indexOf(toMatch)) {
        return;
      }
    }
